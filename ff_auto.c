...
main()
{
    ...
    /* Reading  FAtomes file */
    	npbx=npby=npbz=1;
    	readFAtomes();
	printf("Give total number of frames:\n");
	scanf("%d",&nframes);
...
//	uncoorfile=fopen("uncoor.dat","w");
	//forcefile=fopen("force.dat","w");
	
		sprintf(fName,"PasDeCalcul_Iteration_prot.xyz");
...
//	mass3=14.027e-03;
	...
	fscanf(infile,"%s %lf %lf %lf %lf %lf %lf %s",type[iatomes],&atposreal[0][iat],&atposreal[1][iat],&atposreal[2][iat],&fpos[0][iat], &fpos[1][iat],&fpos[2][iat],id[iatomes]);
       // fprintf(forcefile,"%6s %e %e %e\n",type[iatomes],fpos[0][iat],fpos[1][iat],fpos[2][iat]);
	for (j=0;j<NombreTypesAtomes;j++) {
...
//    	invmass_bead=(double *)malloc(nbeadspmol*sizeof(double));
	fx_acf=(double *)malloc(nframes*sizeof(double ));
	fy_acf=(double *)malloc(nframes*sizeof(double ));
	fz_acf=(double *)malloc(nframes*sizeof(double ));
	fx_CG=(double **)malloc(nbeadstot*sizeof(double *));
	fy_CG=(double **)malloc(nbeadstot*sizeof(double *));
	fz_CG=(double **)malloc(nbeadstot*sizeof(double *));
	for (i=0;i<nbeadstot;i++) { 
	fx_CG[i]=(double *)malloc(nframes*sizeof(double));
	fy_CG[i]=(double *)malloc(nframes*sizeof(double));
	fz_CG[i]=(double *)malloc(nframes*sizeof(double));
	}

/* Loop over frames */

	for (z=0;z<nframes;z=z+1) {
...
printf("Status 1 ok\n");
    	for (iatomes=0;iatomes < natomes;iatomes++) { 
	fscanf(infile,"%s %lf %lf %lf %lf %lf %lf %s",type[iatomes],&atposreal[0][iat],&atposreal[1][iat],&atposreal[2][iat],&fpos[0][iat], &fpos[1][iat],&fpos[2][iat],id[iatomes]);
        fprintf(forcefile,"%6s %e %e %e\n",type[iat],fpos[0][iat],fpos[1][iat],fpos[2][iat]);
}
//printf("Status 2 ok\n");
/*	for (j=0;j<NombreTypesAtomes;j++) {
	if (strcmp(type[iatomes],nomAtome[j])==0) massat[iatomes]=masseAtome[j];
...
printf("Status 2 ok\n");
	...
	// Loop over all beads constituting the chain //
		for (ilamda=0;ilamda<nbeadspmol;ilamda++) {
	//	invmass_bead[ii]=0.0;
	//	mass_bead[ii]=0.0;
		...
		jj=jj+1;
	//	printf("jj=%d\n",jj);
//	        forces des particules C.G.	
		fx_CG[ii][z]=fx_CG[ii][z]+fpos[0][jj];
		//printf("fpos[0][jj]=%e\n",fpos[0][jj]);
		fy_CG[ii][z]=fy_CG[ii][z]+fpos[1][jj];
		fz_CG[ii][z]=fz_CG[ii][z]+fpos[2][jj];
	        }
   		ii=ii+1;
		/* Billes dans la molecule */ 
	//	printf("fx_CG=%e\n",fx_CG[ii][z]);
		}
...
//printf("Status 3 ok\n");
/* Calcul de f_CG acf */
for (ilamda=0;ilamda<nbeadstot;ilamda++) {
	for (i=0;i<z;i++) {
	normx+=fx_CG[ilamda][i]*fx_CG[ilamda][i];
	normy+=fy_CG[ilamda][i]*fy_CG[ilamda][i];
	normz+=fz_CG[ilamda][i]*fz_CG[ilamda][i];
	}
}
normx=normx/nbeadstot;
normy=normy/nbeadstot;
normz=normz/nbeadstot;
printf("normx=%e, normy=%e, normz=%e\n",normx,normy,normz);
//for (ilamda=0;ilamda<nbeadstot;ilamda++) {
	for (i=0;i<z;i++) {
		sumx=0.0;
		sumy=0.0;
		...
		for (j=0;j<z-i;j++) {
			for (ilamda=0;ilamda<nbeadstot;ilamda++) {
			sumx+=fx_CG[ilamda][j]*fx_CG[ilamda][j+i];
			//printf("sumx=%e\n",sumx);
			sumy+=fy_CG[ilamda][j]*fy_CG[ilamda][j+i];
			sumz+=fz_CG[ilamda][j]*fz_CG[ilamda][j+i];
			}
		}
	fx_acf[i]=sumx/nbeadstot;
	fy_acf[i]=sumy/nbeadstot;
	fz_acf[i]=sumz/nbeadstot;
	//printf("sumx=%lf\n",sumx);
}
for (i=0;i<z;i++) {
	fx_acf[i]=fx_acf[i]/normx;
	fy_acf[i]=fy_acf[i]/normy;
	fz_acf[i]=fz_acf[i]/normz;
}
printf("Status 4 ok\n");
	/*  write */
            sprintf(outName,"f_ACF_%d.out",z);
	...
	for (i=0;i<z;i++) {
	fprintf(outfile,"\t%9.6lf \t%9.6lf  \t%9.6lf \t%9.6lf\n",i*dt,fx_acf[i],fy_acf[i],fz_acf[i]); 
	}
fclose(outfile);
printf("Status 5 ok\n");
for (i=0;i<nframes;i++) {
